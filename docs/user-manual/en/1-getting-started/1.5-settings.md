...
Settings → General, "Global Outbound Proxy": configure the HTTP/HTTPS proxy used for outbound requests such as WebDAV/S3 sync and Skills downloads. For security, proxy URLs must not embed `user:pass@` credentials.
