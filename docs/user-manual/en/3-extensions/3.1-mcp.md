...
> **Note**: MCP functionality is supported for Claude and Codex. The Pi core has no native MCP registry, so CC Switch does not generate an unused mirror configuration for it.

### Toggle Implementation

When enabling an app's toggle, CC Switch will:

1. **Update database**: Set the server's `apps.claude/codex` status to `true`
...
2. Select the app to import from (Claude / Codex)
