...
> The directories above follow the per-app directories you configured in Settings.
...
The filter can be combined with search, and the project directory groups can be expanded and collapsed.
...
- CC Switch launches the resume command in the terminal you chose in Settings (CMD / PowerShell / Windows Terminal)
- The terminal opens in the session's project directory
- If terminal launch fails, the command is copied to your clipboard so you can paste it into a terminal

> ⚠️ **Note**: The resumed terminal process reads the environment variables as they are at the moment it starts. A terminal opened after you switched providers carries the new credentials; if you switched just before resuming, make sure the new terminal was really reopened.

> The Resume button is disabled if the session has no resume command available.

#### Directory Picker (Claude Terminal Resume)

**Claude sessions** show a **directory picker** before resume, allowing you to override the default project directory. Useful when:
...
> **Note**: Codex / Pi session resume flows do not yet include the directory picker and still use the session's original project directory.
