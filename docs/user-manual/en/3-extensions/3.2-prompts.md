...
- When switched to Pi, Pi's presets are shown

To use the same prompt across multiple apps, you need to create them separately.

## Import & Export

Exporting configuration (Settings → Advanced → Data Management) includes all presets, which can be restored upon import. The preset content itself never contains keys.
