...
- **Using DeepSeek in Codex: local routing hands-on guide**: walks through adding a Codex provider, enabling local routing, and verifying request forwarding.
...
- **Local Proxy Service** and **Local Routing**: explain the proxy service, live-config takeover, and related risk notes.
