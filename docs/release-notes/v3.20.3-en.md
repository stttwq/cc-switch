...
- **Request Routing**: config rewriting and restoration under takeover — this release's routing fix for a Codex card missing `model_provider` lands on this path.
- **Usage Statistics**: Codex session parsing and pricing configuration, plus this release's byte cursor and pricing backfill rules.
